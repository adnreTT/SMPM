...
onMounted(()=> render()); watch(()=>props.series, ()=>{ if(chart){ chart.destroy() }; render() })
function render(){
  if(!canvas.value) return
  chart = new Chart(canvas.value.getContext('2d'), { type:'bar', data:{
    labels: props.labels,
    datasets:[
...
<style scoped>:host,canvas{ display:block; width:100%; height:280px }</style>

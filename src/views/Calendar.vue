...
function badge(day){
  const match = store.items.find(i => parseInt(i.date.split('-')[2]) === day)
  if(!match) return { cls:'', label:'' }
  const map = {
    'Concluída':'border-success/40 bg-success/10',
    'Pendente':'border-warning/40 bg-warning/10',
    'Atrasada':'border-danger/40 bg-danger/10'
  }

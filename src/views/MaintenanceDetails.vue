  ...
  <div class="bg-white rounded-xl p-4 shadow max-w-3xl">
    ...
    <form class="grid grid-cols-1 md:grid-cols-2 gap-3" @submit.prevent="save">
      ...
      <div>
        ...
        <input v-model="item.date" class="input" required />
      </div>
      <div>
      ...
      <div class="md:col-span-2 flex justify-end gap-2">
        ...
        <button class="px-4 py-2 bg-blue-600 text-white rounded" type="submit">Salvar</button>
      </div>
    </form>
  </div>
</template>
...
const blank = () => ({ id: Math.random().toString(36).slice(2,6), machine:'', date:'', sector:'', status:'Pendente', notes:'' })
...
function save(){
  const exists = store.byId(item.id)
  if(exists) store.update(item.id, item)
  else store.add({ ...item })
  alert('Salvo')
}
</script>
